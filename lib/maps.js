...
export function createMap(db, campaignId, data = {}) {
  ...
  const info = db.prepare(
    'INSERT INTO maps (campaign_id, name, grid_type, grid_size, width, height, cell_feet, fog_mode) VALUES (?,?,?,?,?,?,?,?)'
  ).run(
    campaignId,
    data.name || 'New Map',
...
export function duplicateMap(db, id) {
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(
      'INSERT INTO maps (campaign_id, name, grid_type, grid_size, width, height, background, active, cell_feet, fog_mode) VALUES (?,?,?,?,?,?,?,0,?,?)'
    ).run(
      src.campaign_id,
      `${src.name} (copy)`,
      src.grid_type,
