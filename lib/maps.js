...
export function listMaps(db, sessionId) {
  return db.prepare('SELECT * FROM maps WHERE session_id=? ORDER BY id').all(sessionId);
}
...
export function getActiveMap(db, sessionId) {
  if (sessionId != null) {
    return db.prepare('SELECT * FROM maps WHERE session_id=? AND active=1 ORDER BY id LIMIT 1').get(sessionId);
  }
  return db.prepare('SELECT * FROM maps WHERE active=1 ORDER BY id LIMIT 1').get();
}

export function createMap(db, sessionId, data = {}) {
  const fogMode = ['dungeon','outdoor','none'].includes(data.fog_mode) ? data.fog_mode : 'dungeon';
  const info = db.prepare(
    'INSERT INTO maps (session_id, name, grid_type, grid_size, width, height, cell_feet, fog_mode) VALUES (?,?,?,?,?,?,?,?)'
  ).run(
    sessionId,
    data.name || 'New Map',
...
export function activateMap(db, id) {
  const m = db.prepare('SELECT session_id FROM maps WHERE id=?').get(id);
  if (!m) return false;
  const tx = db.transaction(() => {
    db.prepare('UPDATE maps SET active=0 WHERE session_id=?').run(m.session_id);
...
export function duplicateMap(db, id) {
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(
      'INSERT INTO maps (session_id, name, grid_type, grid_size, width, height, background, active, cell_feet, fog_mode) VALUES (?,?,?,?,?,?,?,0,?,?)'
    ).run(
      src.session_id,
      `${src.name} (copy)`,
      src.grid_type,
...
// in a session (throws). If the deleted map was active, activates another map.
export function deleteMap(db, id) {
  const src = db.prepare('SELECT session_id, active FROM maps WHERE id=?').get(id);
  if (!src) return false;
  const count = db.prepare('SELECT COUNT(*) c FROM maps WHERE session_id=?').get(src.session_id).c;
  if (count <= 1) {
    throw new Error('cannot delete the last map in a session');
  }
  const tx = db.transaction(() => {
    ...
    if (src.active) {
      const next = db.prepare('SELECT id FROM maps WHERE session_id=? ORDER BY id LIMIT 1').get(src.session_id);
